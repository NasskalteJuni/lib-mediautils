...
class Connect extends Listenable() {
    ...
    async _initiateExchange() {
        if(this._locked){
            ...
            this._queued = true;
            if(this._verbose) console.log(this._name, 'is already locked, queue next exchange');
            return;
        }
        this._locked = true;
        if(this._verbose) console.log(this._name, 'locked connection, waiting for accept');
    ...
    _handleLock(){
        if(this._locked){
            if(!this._isYielding){
                ...
                if(this._verbose) console.log(this._name,'not yielding, wait for accept');
                return;
            }else{
                // if not yielding, queue own media changes and continue...
                if(this._verbose) console.log(this._name,'yields, queue own media changes');
                this._queued = true;
            }
        }
        this._locked = true;
        if(this._verbose) console.log(this._name, 'accepts lock');
        this.dispatchEvent('locked', []);
        this._signaller.send(JSON.stringify({
            receiver: this._peer,
            type: 'lock:accept',
    ...
    async _startHandshake(){
        ...
        if(this._verbose) console.log(this._name, 'initiates offer-answer exchange');
    }
    ...
    async _handleSdp(description){
        if(description.type === 'offer'){
            ...
            if(this._verbose) console.log(this._name, 'answers received offer');
        }else{
            await this._connection.setRemoteDescription(description);
            this._signaller.send(JSON.stringify({
                receiver: this._peer,
            ...
            this._locked = false;
            if(this._verbose) console.log(this._name, 'unlocks connection after complete offer-answer exchange');
    ...
    _handleUnlock(){
        this._locked = false;
        if(this._verbose) console.log(this._name, 'unlocking connection');
        this.dispatchEvent('unlock', []);
        if(this._queued) this.dispatchEvent('lockneeded', []);
    }
    ...
    async addMedia(media) {
        ...
        if (arguments.length === 2) {
            ...
            if (media instanceof MediaStream) {
                ...
                queue(track, [new MediaStream([track])]);
